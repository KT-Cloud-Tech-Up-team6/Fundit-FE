...
import { searchProjects, getCategoryProjects, getPublicProject } from "./buyer-project-api.ts";
...
test("카테고리 이름을 인코딩하고 공개 상세 404를 오류로 전달한다", async (t) => {
  let path;
  t.mock.method(globalThis, "fetch", async (url) => {
    path = url;
    return Response.json({ content: [] });
  });
  await getCategoryProjects("테크/가전", "음향 & 기기", "RECENT", 0);
  const url = new URL(path, "https://example.com");
  assert.equal(url.pathname, `/api/v1/categories/${encodeURIComponent("테크/가전")}/projects`);
  assert.equal(url.searchParams.get("categoryMinor"), "음향 & 기기");
