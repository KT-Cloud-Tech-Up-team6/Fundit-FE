...
export function couponPreviewError(preview: OrderPreview, code: string | null): string {
  if (!code) return "";
  const unavailable = preview.unavailableCoupons?.find((coupon) => coupon.couponCode === code);
  if (unavailable) {
  ...
  return preview.appliedCoupons?.some((coupon) => coupon.couponCode === code)
    ? ""
