...
test("스타터 판매가와 주문서 할인 후 금액이 일치한다", () => {
  ...
  const discount = defaultCoupon
    ? couponDiscountAmount(defaultCoupon, totalOrderAmount(summary))
    : 0;
  ...
  assert.equal(item.price - discount, finalPaymentAmount(summary));
});

test("최종 결제 금액은 총 주문 − 총 할인이고 음수면 0으로 막는다", () => {
  assert.equal(finalPaymentAmount(demoPaymentSummary()), 199_000); // 219,900 − 얼리버드 20,900
  assert.equal(
    finalPaymentAmount({
      fundingAmount: 1_000,
      shippingFee: 0,
...
test("적립금 상쇄 가능액은 총 주문 − 쿠폰 할인", () => {
  // 정가 219,900 − 얼리버드 20,900 = 199,000
  assert.equal(maxPointUsage(demoPaymentSummary()), 199_000);
});

test("적립금 사용액은 [0, 보유잔액, 상쇄가능액] 범위로 잘리고 소수·음수·NaN을 정리한다", () => {
  assert.equal(clampPointUsage(3_000, 5_000, 199_000), 3_000);
  assert.equal(clampPointUsage(9_999, 5_000, 199_000), 5_000); // 보유 초과 → 잔액으로
  assert.equal(clampPointUsage(10, 5_000, 3), 3); // 상쇄가능액이 더 작으면 그쪽으로
  assert.equal(clampPointUsage(-100, 5_000, 199_000), 0);
  assert.equal(clampPointUsage(1_500.9, 5_000, 199_000), 1_500); // 정수화
  assert.equal(clampPointUsage(Number.NaN, 5_000, 199_000), 0);
});
...
test("쿠폰은 직접 선택하기 전까지 적용하지 않는다", () => {
  const summary = demoPaymentSummary();
  const coupon = demoCoupons().find((c) => c.id === DEMO_SELECTED_COUPON_ID);
  assert.equal(coupon, undefined);
  assert.equal(summary.couponDiscount, 0);
});
...
test("적립금을 반영하면 최종 결제 금액이 그만큼 줄어든다", () => {
  const base = demoPaymentSummary(); // 최종 199,000 (적립금 0)
