...
export function DelayReasonModal({ open, onClose, onSkip, onSave }: DelayReasonModalProps) {
  return (
    <Modal onClose={onClose} open={open} title="지연 사유 등록">
      <DelayReasonForm key={String(open)} onSave={onSave} onSkip={onSkip} />
    </Modal>
  );
}

/* 날짜는 `DateField`가 제어 컴포넌트라 상태가 필요하다. 폼 안에 두어야 건너뛰기·닫기·ESC로
   나갔다 다시 열 때 다른 입력과 함께 재마운트로 지워진다. */
function DelayReasonForm({ onSkip, onSave }: Pick<DelayReasonModalProps, "onSkip" | "onSave">) {
  const fieldId = useId();
