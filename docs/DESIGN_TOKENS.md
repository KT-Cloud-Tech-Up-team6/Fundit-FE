...
화면 전환은 모바일 1199px 이하, 웹 1200px 이상으로 구분하며 `min-[1200px]:`를 사용한다. 별도 태블릿 디자인은 없다. `md:`와 `lg:`는 간격 등 기존 용도까지 일괄 변경하지 않는다. 모바일 화면의 외곽은 가용 폭을 사용하되 카드·미디어·미리보기의 명시적 치수는 유지한다. 구매자 탐색 화면은 1200px 콘텐츠 폭을, 계정·내역·상세 화면은 793px 콘텐츠 열을 기본으로 사용한다. 아직 반응형이 적용되지 않은 placeholder 화면만 1200px 이상에서 기존 배치를 유지한다.
