...
- `/seller/projects/vacuum-cleaner?tab=funding`에서 최신 화면을 확인합니다. 프로젝트 목록의 기존 목업 ID·제목·썸네일·금액·후원자 수를 재사용하며, 다른 프로젝트에 청소기의 상세 통계를 복제하지 않습니다. 알려지지 않은 ID로 펀딩 현황을 직접 요청하면 404이고, 준비중 프로젝트는 빈 화면을 표시합니다(#324). 상세 목업이 없는 기존 프로젝트는 추가 집계값을 `—`, 리워드 표를 정보 없음으로 표시합니다.
...
- 지시문 `1230:19039`에 따라 예약하지 않으면 날짜·시각을 현재 로컬 시간으로 표시하고 비활성화합니다. 모달을 열거나 예약을 해제할 때 갱신합니다. 예약 입력은 로컬 상태였고 서버 예약·임시저장·불러오기는 #326에서 연결했습니다.
...
- #246에서 화면 정의 `1299:32809`와 9월 21일 이력 `1230:24887`을 반영했습니다. `/seller/live/demo-live/console`의 `1299:32829`는 송출 전 준비 단계가 아닌 로딩 미리보기입니다. 모델·Storybook 이름은 `loading`/`Loading`이며 임의 타이머로 송출 성공을 만들지 않습니다. 콘솔 전용 헤더의 secondary `LIVE 종료`는 로딩·송출 중 상태에서 기존 종료 모달 `1299:32969` 및 LIVE 체크 흐름으로 연결됩니다. 일반 판매자 화면의 참여자 전환은 유지합니다. 이는 로컬 목업 종료이며 실제 방송 종료 API 연결은 미병합 #230과 별도 통합해야 합니다. 생성 모달의 LIVE 시작·리허설·설정과 실제 송출은 기존 연동 대기 범위입니다.
...
- #267에서 LIVE 체크 추가 완료 모달 `1993:43976`을 반영했습니다. 기존 토스트(`setNotice`)를 대체하며, `나가기`는 모달만 닫고 `상세페이지로`는 `/projects/{projectId}?tab=live-proof`로 이동합니다. 콘솔이 아는 값은 `liveId`뿐이라 `projectId`는 구매자 쪽 데모 매핑(`getLiveDemoConnection`)에서 빌립니다.
...
- #246 검증: 1440px·390px 실제 URL에서 큐시트 접기·펼치기/이전·다음, 로딩 중 종료·빈 LIVE 체크·목록 복귀·뒤로 가기를 확인했습니다. 송출 중 채팅→종료→답변 선택→LIVE 체크 추가는 Storybook 목업으로 검증했습니다. 우측 패널 배경은 투명, 다음 버튼은 secondary로 이미 일치했습니다. 프로젝트 관리 `1231:14050/14157/14186`의 세 탭 첫 카드 위치도 각각 244px(1440px), 409px(390px)로 같아 별도 간격 보정을 추가하지 않았습니다. 인증·목록 응답은 격리된 브라우저의 fixture이며 실제 API 검증이 아닙니다. 상태 모델 6개, lint·typecheck·build·포맷 검사를 통과했습니다. 두 섹션의 텍스트·이력·화면 정의·prototype reaction과 annotation을 확인했고 별도 `interaction_spec` 노드는 없었습니다. 비공개 plugin data는 도구에서 지원하지 않아 확인하지 못했습니다.
- #320에서 UUID 콘솔(`/seller/live/{UUID}/console`)을 데모와 같은 Figma 패널로 바꾸고 실제 API에 연결했습니다. 공용 패널은 `features/live-console/ui`의 `console-panels`·`question-manager`·`live-check-flow`이고 실제 경로는 `features/live-integration/ui/real-seller-console.tsx`입니다. 질문 요약(미답변·답변 완료), 원문, 선택 즉시 AI 초안·재생성·근거, 초안 없음 경고 카드, 채팅 보내기(답변 등록), 집계된 Q&A(답변 주체 AI 자동답변·판매자), AI 준비 중과 모인 질문 없음 문구 분리, 30초 갱신과 갱신 시각, 송출 영상·시청자 수·경과 시간, 큐시트 패널, LIVE 종료 → LIVE 체크 추가 → 상세페이지 LIVE 체크 탭을 연결했습니다. API는 [API_CONTRACT.md 5.8](./API_CONTRACT.md#58-판매자-live-콘솔-320)에 적습니다.
- #320의 목업 유지 항목(2026-09-23 결정): 답변 완료 처리, 스트림 상태 확인, 방송 중 펀딩 건수·금액, 판매자 채팅은 BE API가 없어 Figma 자리에 두고 누르면 "준비 중"을 안내합니다. 수치는 `-`입니다. 채팅 보내기는 답변 등록만 하고 채팅 게시는 IVS 연동 후입니다. 서버 원문에는 작성자 이름이 없어 원문은 내용만 표시합니다. 이 가운데 방송 중 주문 건수·금액은 #342에서 `GET /api/v1/orders/live-stats`의 결제 완료 값으로 연결했습니다([API_CONTRACT.md](./API_CONTRACT.md) 5.8). Figma 작업 이력의 마지막 수정(9/22 `1993:44419`)은 #267에서 반영했고 이후 변경은 없습니다.
- #326에서 LIVE 스튜디오 목록과 LIVE 생성을 남은 API에 연결했습니다. `/seller/live`는 탭마다 해당 상태만 조회하고(준비중 = `status=DRAFT,SCHEDULED`) 탭 건수를 `GET /lives/status-counts`로 채웁니다. 검색창은 `?search=`를 `q`로 넘기며 상태·페이지와 함께 URL에 남아 뒤로 가기로 되돌아갑니다. 생성 확인 화면의 `LIVE 시작`은 `POST /lives/{liveId}/start` 성공 시 `/seller/live/{liveId}/console`로 이동하고, 409는 "이미 시작했거나 종료된 LIVE"로 구분해 안내합니다. 연타는 ref 가드로 한 번만 보냅니다. `불러오기`는 이 프로젝트의 임시저장(DRAFT)을 골라 `GET /lives/{liveId}`로 같은 liveId에 이어 씁니다 — 고르는 화면이 원본에 없어 공용 Modal로 최소 구현했습니다. 큐시트 화면의 프로젝트 조회도 `/lives/mine` 훑기 대신 단건 조회를 씁니다. 영상 송출 정보(스트림 키)는 여전히 조회 API가 없어 확인 화면에서 그 사실을 안내합니다.
- #344에서 위 생성 확인 화면의 "스트림 키 미제공" 안내 자리에 송출 주소와 스트림 키(`GET /lives/{liveId}/stream-info`)를 보여 줍니다. Figma 판매자 LIVE 흐름에는 이 자리가 없어 기존 안내 위치를 썼습니다. 키는 기본으로 가리고 보기·복사를 둡니다([API_CONTRACT.md 5.9](./API_CONTRACT.md#59-판매자-live-스튜디오-목록시작-326)).
- #320 검증: 격리된 Playwright로 API를 가로채고 로컬 mp4를 재생해 1440px·390px에서 위 흐름, 일부 등록 실패 후 재시도, 상세페이지 이동과 뒤로 가기, 조회 실패·권한 없음·종료 409, 30초 갱신을 확인했습니다. 데모 콘솔 스토리 14개도 두 폭에서 통과합니다. 실제 BE·AI·IVS와의 연동은 확인하지 못했습니다.
...
| `/live/[liveId]`                       | LIVE 방송·채팅·다시보기 | read public, write member | live / replay / clip implemented (UUID API)             |
...
| `/payment/[orderId]`            | Toss 결제위젯 결제      | member             | implemented |
...
실제 결제는 `/payment/[orderId]`(주문 UUID)에서 Toss 결제위젯 SDK로 진행합니다. 주문 생성 직후와 참여 내역 상세의 결제 대기 주문(`결제하기`)이 이 화면으로 들어옵니다. 결제창은 리다이렉트 방식이며 성공·실패 모두 `/payment/result`로 복귀합니다. 복귀 쿼리에 `paymentKey`가 있으면 승인(confirm)을 한 번 호출하고, `code`가 있으면 실패·취소 화면을 보여 줍니다. 승인 성공 후 주문 상태(`FUNDING_IN_PROGRESS`)는 BE에서 비동기로 반영되므로 반영을 확인한 뒤 `/payment/result?orderId={UUID}`로 교체 이동하고, 그 전에는 재결제 진입을 숨깁니다. 쿼리가 없으면 기존처럼 주문 UUID 조회 또는 데모 완료 화면입니다. 코드 연결은 끝났지만 실제 Toss 샌드박스·게이트웨이 통합 확인은 아직 남아 있습니다.
...
인용구 컨트롤은 #259에서 제거했습니다. BE `RichTextSanitizer`가 blockquote를 보존하지 않아 저장이 거부되기 때문이며, 자세한 근거는 [서식 저장 계약](./API_CONTRACT.md#프로젝트-스토리의-서식-저장-계약-234)에 있습니다. #166에서 고쳤던 전체 선택 후 인용구 적용 시 활성 표시가 꺼지던 문제도 컨트롤과 함께 없어졌습니다. 원본 섹션의 화면 정의·파일 선택 지시·아이콘 상태·히스토리를 재확인했으며 섹션 내부 prototype reaction은 없습니다.
...
미리보기는 최신 Figma 화면(`1403:37702`, 모달 `1403:37752`)과 판매자 IA PDF 4페이지의 모달 정의를 반영합니다. 공용 `Modal`의 996px 폭과 672px 높이를 사용하고 작은 화면에서는 뷰포트에 맞춰 축소합니다. 원본의 모달 내부는 자리표시자이며, 사용자에게 전달된 상세페이지 미리보기 요구에 따라 기존 `BuyerProjectDetail`을 재사용합니다. 현재 작성한 제목·썸네일·본문을 상세페이지에 반영합니다. #332부터 서버에서 불러온 프로젝트는 판매자·목표 금액·달성률·참여자·남은 기간을 소유자 미리보기 응답(`GET /api/v1/projects/{projectId}/preview`, 공개 상세와 같은 `ProjectDetailResponse`)으로 채우고, 리워드는 판매자 리워드 조회(`/rewards/mine`)로 구매자 상세와 같은 표시(`RewardSummary`)를 재사용합니다. 리워드는 구매자 모바일 상세처럼 소개 아래에 놓이며 불러오는 중·실패(다시 시도) 상태를 안내합니다. 안내 문구는 저장된 프로젝트 정보임을 알리고, 데모 프로젝트에서는 기존 예시 데이터 안내를 유지합니다. 편집기와 동일한 Tiptap 확장을 사용해 서식·이미지·영상을 유지하며, 닫기·Esc·배경 클릭으로 작성 화면에 복귀하고 다시 열면 최신 내용을 반영합니다. URL 이동은 없으며 기존 `/preview` placeholder는 사용하지 않습니다. 미리보기의 뒤로가기·탭 이동·공유·찜·후원 버튼은 비활성화하고, 하단 후원 영역은 모달 내부에 배치합니다. 실제 구매자 페이지의 기존 동작은 유지합니다. 임시저장·저장 CTA의 동작은 [#322 프로젝트 공개](./PROJECT_BASIC_INFO.md#스토리-저장-시-프로젝트-공개-322)를 따릅니다. 준비중 프로젝트의 탭 제한, AI 결과 복사하기와 재생성 시 입력 초기화는 IA와 현재 목업 사이의 남은 차이이며 이번 작업에서 변경하지 않습니다. AI 대화·생성 결과는 기존 편집기 내부 모달입니다.
...
- seller 화면은 회원 인증을 확인합니다. 프로젝트 개인정보 동의는 신규 생성마다 별도로 받아 생성 직후 서버에 기록하며, 공개할 때 BE가 기록 여부를 검증합니다(#322).
...
- IA v1.3 `FL_S_PR_LIST`(`J5`)와 2026-09-23 PM 결정에 따라 상태와 관계없이 카드의 프로젝트명은 스토리 작성(`?tab=story`), 관리 버튼은 펀딩 관리(`?tab=funding`)로 이동합니다. 이전에는 둘이 같은 곳(준비중 기본 정보, 진행중 펀딩 관리, 종료 제작·배송)으로 이동했습니다. IA의 "우측 수정 아이콘"은 Figma 목록 `1231:14936`에 없어 두지 않습니다.
- 준비중 프로젝트는 펀딩이 시작되지 않아 펀딩 관리 탭에서 집계 API를 부르지 않고 빈 화면(야자수 그래픽과 "펀딩 내역이 없습니다")을 표시합니다. PM 요청(2026-09-23)대로 제작·배송의 잠긴 단계 안내와 같은 공용 `EmptyState` 구성이며 버튼은 없습니다. 데모 준비중 프로젝트도 404 대신 같은 화면을 표시합니다.
- 준비중 카드는 IA에 없는 오픈 예정일·마지막 수정일을 표시하지 않습니다(2026-09-23 결정). 진행중 카드는 `fundingDeadline`으로 남은 기간 `D-N`을 표시하며, 펀딩 관리 화면과 같게 BE `remainingDays` 공식(남은 시간의 일수 + 1, 지나면 "종료")을 따릅니다.
