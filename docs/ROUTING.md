...
- 구매자 LIVE 메인은 [구현 범위와 확인 방법](./BUYER_LIVE_MAIN.md)을 참고한다. `/live`와 `/live/upcoming`을 전용 `(buyer-live)` 그룹에 두고 다른 구매자 화면은 기존 BuyerShell을 유지한다. 카테고리 메뉴는 목적지가 미정이므로 비활성 상태다.
