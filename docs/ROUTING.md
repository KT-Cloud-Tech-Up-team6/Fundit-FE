...
- 구매자 카테고리 리스트 `/categories/[slug]`는 전용 헤더·하단 탭을 사용하는 `(buyer-category)` 그룹에 둔다. 현재 카테고리와 소분류는 화면 확인용 목업이다. `BuyerBottomNavigation`의 카테고리 탭은 `/categories/tech-appliances`(첫 번째 카테고리)로 진입한다. 진입 시 현재 경로를 `sessionStorage`(`buyer-category-return-path`)에 기록해두고, 카테고리 화면이 이미 활성 상태일 때 같은 탭을 다시 누르면 그 경로로 돌아간다 — 브라우저 `history`가 아니라 앱 내에서 직접 기록한 경로이므로 외부 링크·검색결과로 진입했을 때도 안전하다. 이 기록은 카테고리 탭 재클릭, 홈·라이브·마이 탭 클릭, 브라우저 뒤로가기·앞으로가기(`popstate`)에서 함께 정리되므로 이전 방문의 값이 이후 무관한 진입에 재사용되지 않는다. 좌측 SNB로 카테고리(slug)만 바꾸는 이동은 `replace`라 `popstate`가 발생하지 않고, Next.js RSC 특성상 이 과정에서 컴포넌트가 리마운트되더라도 기록을 지우지 않으므로 카테고리 내부를 여러 번 옮겨다녀도 값이 보존된다(마운트/언마운트 자체는 정리 신호로 쓰지 않는다). 기록이 없으면(예: 앱 내에서 카테고리 탭을 거치지 않고 처음 들어온 딥링크) 홈으로 대체한다. `sessionStorage`는 새로고침에도 유지되므로, 카테고리 화면에서 새로고침한 뒤 카테고리 탭을 눌러도 기록해둔 복귀 경로로 정상 이동한다 — 새로고침 자체는 복귀 기록을 무효화하지 않는다. 좌측 SNB에서 카테고리를 바꾸는 이동은 `replace`를 써서 히스토리를 쌓지 않으므로, 화면 안에서 카테고리를 여러 번 바꿔도 카테고리 탭 재클릭 한 번으로 진입 전 화면까지 돌아간다 — 실제 진입 slug·카테고리 체계는 여전히 목업이다. 소분류명을 누르면 `/categories/[slug]/[subcategorySlug]`(소분류 결과 목록)로 이동하며, 이 화면은 아직 `PagePlaceholder`다.
