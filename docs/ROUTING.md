...
- #320의 목업 유지 항목(2026-09-23 결정): 답변 완료 처리, 스트림 상태 확인, 방송 중 펀딩 건수·금액, 판매자 채팅은 BE API가 없어 Figma 자리에 두고 누르면 "준비 중"을 안내합니다. 수치는 `-`입니다. 채팅 보내기는 답변 등록만 하고 채팅 게시는 IVS 연동 후입니다. 서버 원문에는 작성자 이름이 없어 원문은 내용만 표시합니다. Figma 작업 이력의 마지막 수정(9/22 `1993:44419`)은 #267에서 반영했고 이후 변경은 없습니다.
