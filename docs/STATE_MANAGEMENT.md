...
탭 간에는 localStorage의 `fundit.auth.session-generation`에 임의 세대 표식만 공유한다. 토큰·계정 ID·권한은 저장하지 않는다. 다른 탭의 세대 변경은 storage 이벤트 및 요청 전후의 직접 읽기로 감지한다. 변경을 감지하면 기존 토큰·사용자 상태를 비우고(토큰이 있던 탭은 Query 캐시까지 비운다), 대기/진행 중인 refresh 및 이전 세대의 인증 요청 결과는 AbortError로 폐기한다. 토큰이 없던 탭(비로그인 방문의 세션 복구 실패 포함)은 지울 회원 데이터가 없어 Query 캐시를 비우지 않는다. 비우면 진행 중인 요청까지 지워져 같은 요청이 한 번 더 나가고, 조회 수처럼 호출마다 집계되는 API가 두 번 잡힌다(#338). 다른 탭은 새 계정을 자동 적용하지 않고 비로그인 상태로 전환하며, 새로고침 시 서버 쿠키로 세션을 복구한다. 다만 로그아웃(#254)은 서버 쿠키까지 폐기하므로 이 복구가 되지 않는다. 일반적인 같은 계정의 refresh는 세대를 바꾸거나 다른 탭을 로그아웃시키지 않는다.
