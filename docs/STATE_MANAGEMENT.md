...
Server Component의 `fetch`는 Next.js 16 기본값인 비영속 조회를 사용합니다. 동일한 렌더 트리의 같은 요청은 memoization할 수 있지만 요청 간 공유 캐시로 간주하지 않습니다. 공개 데이터에 요청 간 캐시가 필요하면 데이터 계약에서 최신성, 만료와 무효화 조건을 먼저 정하고 해당 데이터 함수에 `cache`, `revalidate`, tag 또는 `use cache` 정책을 명시합니다. 사용자별 데이터는 이 공유 캐시에 넣지 않습니다.
...
- 로그아웃이 서버에서 확정되면 화면 이동 전에 Query Client의 전체 캐시를 제거합니다.
- 같은 브라우저에서 계정을 전환할 때도 이전 세션의 전체 캐시를 먼저 제거한 뒤 새 사용자 데이터를 조회합니다.
...
- TanStack Query 도입 PR은 로그아웃·계정 전환 후 이전 사용자 데이터가 표시되지 않는 회귀 테스트를 포함합니다.
